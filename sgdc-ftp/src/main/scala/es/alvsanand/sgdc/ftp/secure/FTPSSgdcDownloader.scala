...
import es.alvsanand.sgdc.ftp._
...
/**
  * The parameters for es.alvsanand.sgdc.ftp.secure.FTPSSgdcDownloader.
  *
...
/**
  ...
  */
private[secure]
class FTPSSgdcDownloader(parameters: FTPSParameters)
  extends SgdcDownloader[FTPSlot, FTPSParameters](parameters) {
  ...
  private def connect(): Unit = {
    if (!client.isConnected) {
      ...
      if (!FTPReply.isPositiveCompletion(reply)) {
        throw SgdcDownloaderException(s"Error connecting to server: $reply")
      }
      if (!client.login(parameters.cred.user, parameters.cred.password.getOrElse(""))) {
        throw SgdcDownloaderException(s"Error logging in with user[${parameters.cred.user}]")
      }
