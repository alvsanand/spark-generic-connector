...
class FTPSSgdcDownloaderTest extends FlatSpec with Matchers with OptionValues with Inside
  ...
  it should "fail with obligatory parameters" in {
    a[IllegalArgumentException] shouldBe
      thrownBy(new FTPSSgdcDownloader(FTPSParameters(null, 21, null, null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new FTPSSgdcDownloader(FTPSParameters("host", 21, null, null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new FTPSSgdcDownloader(FTPSParameters("host", 21, "dir", null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new FTPSSgdcDownloader(FTPSParameters("host", 21, "dir", FTPCredentials(null))))
    a[IllegalArgumentException] shouldBe
      thrownBy(new FTPSSgdcDownloader(FTPSParameters("host", 21, "dir", FTPCredentials(null),
  ...
  it should "work with obligatory parameters" in {
    noException should be thrownBy(
      new FTPSSgdcDownloader(FTPSParameters("host", 21, "dir", FTPCredentials("user"),
