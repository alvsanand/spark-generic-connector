...
class FTPSgdcDownloaderFactoryTest extends FlatSpec with Matchers with OptionValues
  ...
  it should "work with obligatory parameters" in {
    noException should be thrownBy(
      FTPSgdcDownloaderFactory.get(FTPParameters("host", 21, "directory", FTPCredentials("user")))
      )
    noException should be thrownBy(
      FTPSgdcDownloaderFactory.get(FTPParameters("host", 21, "directory", FTPCredentials("user"),
  ...
  it should "work with proxy parameters" in {
    ...
    FTPSgdcDownloaderFactory.get(parameters).asInstanceOf[FTPSgdcDownloader].usesProxy() should
      be(true)
