...
class FTPSSgdcDownloaderFactoryTest extends FlatSpec with Matchers with OptionValues
  with Inside with  Inspectors with BeforeAndAfterAll {

  it should "fail with obligatory parameters" in {
    a[IllegalArgumentException] shouldBe thrownBy(FTPSSgdcDownloaderFactory
  ...
  it should "work with obligatory parameters" in {
    noException should be thrownBy(
      FTPSSgdcDownloaderFactory.get(FTPSParameters("host", 21, "dir", FTPCredentials("user"),
