...
class FTPSgdcDownloaderTest extends FlatSpec with Matchers with OptionValues with Inside
  ...
  it should "work with obligatory parameters" in {
    noException should be thrownBy(
      new FTPSgdcDownloader(FTPParameters("host", 21, "directory", FTPCredentials("user")))
      )
    noException should be thrownBy(
      new FTPSgdcDownloader(FTPParameters("host", 21, "directory", FTPCredentials("user"),
  ...
  it should "work with proxy parameters" in {
    ...
    noException should be thrownBy(new FTPSgdcDownloader(parameters))
    new FTPSgdcDownloader(parameters).asInstanceOf[FTPSgdcDownloader].usesProxy() should
      be(true)
