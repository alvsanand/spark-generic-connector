...
class SFTPSgdcDownloaderTest extends FlatSpec with Matchers with OptionValues with Inside
  ...
  it should "fail with obligatory parameters" in {
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgdcDownloader(SFTPParameters(null, 22, null, null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgdcDownloader(SFTPParameters("host", 22, null, null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgdcDownloader(SFTPParameters("host", 22, "dir", null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgdcDownloader(SFTPParameters("host", 22, "dir", FTPCredentials(null))))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgdcDownloader(SFTPParameters("host", 22, "dir", FTPCredentials(null),
  ...
  it should "work with obligatory parameters" in {
    noException should be thrownBy (
      new SFTPSgdcDownloader(SFTPParameters("host", 22, "dir", FTPCredentials("user"),
  ...
  it should "work with test user and empty/not existing directory" in {
    ...
    val downloader = new SFTPSgdcDownloader(parameters)
  ...
  it should "fail because invalid user" in {
    ...
    a[es.alvsanand.sgdc.core.downloader.SgdcDownloaderException] shouldBe
  ...
  it should "work with encrypted private key" in {
    ...
    it should "fail with bad name" in {
      ...
      a[SgdcDownloaderException] should be thrownBy(downloader.download(FTPSlot(fileName, new Date),
        out))
    }
}
