...
class SFTPSgdcDownloaderFactoryTest extends FlatSpec with Matchers with OptionValues
  with Inside with  Inspectors with BeforeAndAfterAll {

  it should "fail with obligatory parameters" in {
    a[IllegalArgumentException] shouldBe thrownBy(SFTPSgdcDownloaderFactory
  ...
  it should "work with obligatory parameters" in {
    noException should be thrownBy(
      SFTPSgdcDownloaderFactory.get(SFTPParameters("host", 21, "dir", FTPCredentials("user"),
