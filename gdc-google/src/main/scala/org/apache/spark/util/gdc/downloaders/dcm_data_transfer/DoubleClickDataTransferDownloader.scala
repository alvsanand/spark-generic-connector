...
/**
  ...
  */
class DoubleClickDataTransferDownloader(credentialsPath: String, bucket: String, types: Seq[DoubleClickDataTransferFileType] = Seq.empty) extends Downloader[DoubleClickDataTransferFile] {
  ...
  def list(): Seq[DoubleClickDataTransferFile] = {
    ...
    Try({
      ...
      files.flatMap(x => DoubleClickDataTransferFileTypes.getDoubleClickDataTransferFile(x.getName)).filter(f => types.isEmpty || types.contains(f.ddtFileType.getOrElse(None))).toSeq
    })
    match {
      case Success(v) => v
  ...
  def downloadFile(file: DoubleClickDataTransferFile, out: OutputStream): Unit = {
    Try({
  ...
  private def builder(): Storage = synchronized {
    if (_builder == null) {
      logDebug(s"Initiating DoubleClickDataTransferDownloader[credentialsPath: $credentialsPath, bucket: $bucket]")
      ...
      logDebug(s"Initiated DoubleClickDataTransferDownloader[credentialsPath: $credentialsPath, bucket: $bucket]")
    }
...
object DoubleClickDataTransferDownloader {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val downloader = new DoubleClickDataTransferDownloader(credentialFile, bucket)
    downloader.list().foreach(println)
    downloader.downloadFile(DoubleClickDataTransferFile(args(2)), new FileOutputStream(File.createTempFile(file, ".tmp")))
  }
}