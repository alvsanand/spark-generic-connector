...
class DataTransferFileTypesTest extends FlatSpec with Matchers with OptionValues with Inside with
  ...
  it should "work getDoubleClickDataTransferFile" in {
    ...
    val dateA = Option(sd.parse("2016-09-27 00:00:00:000"))
    val typeA = Option(DataTransferFileTypes.ACTIVITY)
    val dtfA = Option(new DataTransferFile(fileA, dateA, typeA))
    ...
    val dateB = Option(sd.parse("2016-09-26 02:00:00:000"))
    val typeB = Option(DataTransferFileTypes.CLICK)
    val dtfB = new DataTransferFile(fileB, dateB, typeB)
    ...
    val dateC = Option(sd.parse("2016-09-26 03:00:00:000"))
    val typeC = Option(DataTransferFileTypes.IMPRESSION)
    val dtfC = new DataTransferFile(fileC, dateC, typeC)
