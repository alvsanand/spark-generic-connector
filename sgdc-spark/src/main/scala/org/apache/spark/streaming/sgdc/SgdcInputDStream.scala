...
import es.alvsanand.sgdc.core.util.Retry
import org.apache.spark.rdd.RDD
import org.apache.spark.streaming.dstream._
import org.apache.spark.streaming.sgdc.SgdcRange.{SgdcDateSlotsRange, SgdcDateRange}
 ...
 *
  ...
  * @param fromSgdcRange The range from where the SgdcInputDStream will begin.
  ...
  */
private[sgdc]
final class SgdcInputDStream[A <: SgdcDateSlot: ClassTag, B <: SgdcDownloaderParameters: ClassTag](
                                                  ssc_ : StreamingContext,
                                                  sgdcDownloaderFactory: SgdcDownloaderFactory[A, B],
                                                  parameters: B,
                                                  fromSgdcRange:
                                                  Option[SgdcRange] = None,
                                                  charset: String = "UTF-8",
                                                  maxRetries: Int = 3)
  extends InputDStream[String](ssc_) {
  private[streaming] override val checkpointData = new SgdcInputDStreamCheckpointData
  @transient private var _sgdcDownloader: SgdcDownloader[A, B] = null
    ...
    Retry(maxRetries) {
      ...
      case Success(list) => {
        ...
        if (newSlots.length == 0) {
          ...
          val metadata = Map(
            "newSlots" -> newSlots,
            "sgdcDownloaderParameters" -> parameters,
            "fromSgdcRange" -> fromSgdcRange,
  ...
  /**
    * Internal method that instantiate a es.alvsanand.sgdc.core.downloader.SgdcDownloader.
    * @return
    */
  private def sgdcDownloader = {
    if (_sgdcDownloader == null) {
      _sgdcDownloader = sgdcDownloaderFactory.get(parameters)
    }
    _sgdcDownloader
  }

  /**
    * Internal method that check if a es.alvsanand.sgdc.core.downloader.SgdcSlot has not been
    * processed before
 *
  ...
  private def isNewSlot(slot: A): Option[A] = {
    ...
    val validRange: Boolean = fromSgdcRange match {
      case Some(r: SgdcDateRange) =>
        (r.date == null ||
          (slot.date!=null
            && r.date.getTime <= slot.date.getTime)
          )
      case Some(r: SgdcDateSlotsRange) =>
    ...
    if (!validRange) {
      logDebug(s"$slot rejected because it is not in range[${fromSgdcRange.get}]")
      return None
    }
  ...
  /**
    * Internal method that check if a es.alvsanand.sgdc.core.downloader.SgdcSlot
 *
    * @param slots
    * @return
    */
  private def slotsToRDD(slots: Array[A]): RDD[String] = {
    new SgdcRDD[A, B](context.sparkContext, slots, sgdcDownloaderFactory,
  ...
  /**
    ...
    */
  private[streaming]
  class SgdcInputDStreamCheckpointData extends DStreamCheckpointData(this) {
