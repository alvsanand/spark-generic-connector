...
package es.alvsanand.sgc.google
...
import es.alvsanand.sgc.core.util.{IOUtils, Logging}
...
object GoogleHelper extends Logging {
  ...
  def checkCredentialsDirectory(directory: File): Unit = {
    ...
    if(!REQUIRED_CREDENTIALS_FILES.forall(currentFiles.contains(_))) {
      ...
      throw SgcConnectorException(msg)
    }
  }

  /**
    * Create a Credentials zip used by
    * es.alvsanand.sgc.google.cloud_storage.CloudStorageSgcConnector and
