...
class CloudStorageSgcConnectorFactoryTest extends FlatSpec with Matchers with OptionValues
  with Inside with  Inspectors with BeforeAndAfterAll {

  it should "fail with empty parameters" in {
    a[IllegalArgumentException] shouldBe thrownBy(CloudStorageSgcConnectorFactory
  ...
  it should "work with obligatory parameters" in {
    CloudStorageSgcConnectorFactory.get(CloudStorageParameters("FOO", "FOO")) should not be (null)
  }
}
