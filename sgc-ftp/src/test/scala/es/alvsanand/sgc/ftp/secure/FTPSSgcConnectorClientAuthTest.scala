...
class FTPSSgcConnectorClientAuthTest extends FlatSpec with Matchers with OptionValues with Inside
  ...
  it should "fail because empty keystore" in {
    ...
    a[SgcConnectorException] shouldBe thrownBy(connector.list().map(_.name))
  }
