...
class SFTPSgcConnectorTest extends FlatSpec with Matchers with OptionValues with Inside
  ...
  it should "fail with obligatory parameters" in {
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgcConnector(SFTPParameters(null, 22, null, null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgcConnector(SFTPParameters("host", 22, null, null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgcConnector(SFTPParameters("host", 22, "dir", null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgcConnector(SFTPParameters("host", 22, "dir", FTPCredentials(null))))
    a[IllegalArgumentException] shouldBe
      thrownBy(new SFTPSgcConnector(SFTPParameters("host", 22, "dir", FTPCredentials(null),
  ...
  it should "work with obligatory parameters" in {
    noException should be thrownBy (
      new SFTPSgcConnector(SFTPParameters("host", 22, "dir", FTPCredentials("user"),
  ...
  it should "fail because invalid user" in {
    ...
    a[es.alvsanand.sgc.core.connector.SgcConnectorException] shouldBe
      thrownBy(connector.list())
  }
  ...
  it should "work with encrypted private key" in {
    ...
    it should "fail with bad name" in {
      ...
      a[SgcConnectorException] should be thrownBy(connector.fetch(FTPSlot(fileName, new Date),
        out))
    }
}
