...
package es.alvsanand.sgdc.google
...
import es.alvsanand.sgdc.core.util.{IOUtils, Logging}
...
object GoogleHelper extends Logging {
  ...
  def checkCredentialsDirectory(directory: File): Unit = {
    ...
    if(!REQUIRED_CREDENTIALS_FILES.forall(currentFiles.contains(_))) {
      ...
      throw SgdcDownloaderException(msg)
    }
  }

  /**
    * Create a Credentials zip used by
    * es.alvsanand.sgdc.google.cloud_storage.CloudStorageSgdcDownloader and
