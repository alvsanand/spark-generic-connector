...
import es.alvsanand.sgdc.google
...
class DataTransferSgdcDownloaderTest extends FlatSpec with Matchers with OptionValues with Inside
  with
  ...
  private def createDownloaderList(files: List[String],
                                   types: Seq[DataTransferFileType] = Seq.empty):
                                    DataTransferSgdcDownloader = {
    val downloader = new DataTransferSgdcDownloader(DataTransferParameters("FOO", "FOO", types))
  ...
  private def createDownloaderFile(file: String): DataTransferSgdcDownloader = {
    val downloader = new DataTransferSgdcDownloader(DataTransferParameters("FOO", "FOO"))
  ...
  it should "fail with empty parameters" in {
    a[IllegalArgumentException] shouldBe
      thrownBy(new DataTransferSgdcDownloader(DataTransferParameters("FOO", null)))
    a[IllegalArgumentException] shouldBe
      thrownBy(new DataTransferSgdcDownloader(DataTransferParameters("FOO", "")))
    a[IllegalArgumentException] shouldBe
      thrownBy(new DataTransferSgdcDownloader(DataTransferParameters(null, "FOO")))
    a[IllegalArgumentException] shouldBe
      thrownBy(new DataTransferSgdcDownloader(DataTransferParameters("", "FOO")))
  }
